...
const ExpenseForm = ({ expenseData, isOpen, onClose, refreshData }) => {
  ...
  useEffect(() => {
    if (expenseData && isOpen) {
      ...
      const { category: cat, description: desc } = decodeDescription(
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const res = await axios.post("/expenseTable/api/categories", {
  ...
  const handleUpdateCategory = async (id) => {
    if (!editingName.trim())
      return showAlertMessage({ message: "Enter a name", type: "error" });
    try {
      const res = await axios.put(`/expenseTable/api/categories/${id}`, {
            ...
            {catLoading ? (
              ...
              ["expense", "income"].map((t) => (
                <div key={t} className={styles.catTypeSection}>
                  <p className={styles.catTypeLabel}>
